    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, 20):
            ...
            for iteration, data in enumerate(self.loaders.train_loader):
                ...
                source_images, target_images = source_images.type(torch.FloatTensor), target_images.type(torch.FloatTensor)

                #########################################################################################################
                #                                                     Generator                                         #
                #########################################################################################################
                fake_images = self.generator(source_images)
                # print(fake_images.device, target_images.device)
                gan_loss = self.criterion_GAN(self.discriminator(fake_images, source_images), ones)
                recon_loss = self.criterion_recon(fake_images, target_images.to(self.device))
